...
public class Main {
    ...
    /* docstring*/
    static void StoneGame() throws IOException{
        ...
        while (T-- > 0) {
            ...
            if(maxIndex > minIndex){
                first_answer = maxIndex+1;
                second_answer = (n-maxIndex)+(minIndex+1);
                third_answer = (n-minIndex);
            }
            if(maxIndex < minIndex){
                first_answer = minIndex+1;
                second_answer = (n-minIndex)+(maxIndex+1);
                third_answer = (n-maxIndex);
            }
            int ans = Math.min(second_answer,Math.min(first_answer,third_answer));
    ...
    /* docstring*/
    static void FriendsCandies() throws IOException{
        int T = Integer.parseInt(sc.next());
        while (T-- > 0) {
            // Write Your Code...
            int n = sc.nextInt();
    ...
    /* docstring*/
    static void NumberPairs() throws IOException{
        int T = Integer.parseInt(sc.next());
        while (T-- > 0) {
            // Write Your Code...
            int n = sc.nextInt();
            ...
            for(int i=0;i<n;i++){
                for(int j=n-1;j>i;j--){
                    if((l <= arr[i]+arr[j]) && (arr[i]+arr[j]<=r)){
                        count++;
                    }
                    else{
                        break;
