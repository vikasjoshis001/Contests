...
class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            while (T-- > 0) {
                ...
                rem2 = n%8;
                rem3 = n%10;
                int min = Math.min( (int) rem1,(int) Math.min( (int) rem2,(int) rem3));
                if(min==rem1){
                    fop.println(((n/6)+1)*15);}
                if(min==rem2){fop.println(((n/8)+1)*20);}
                if(min==rem3){fop.println(((n/10)+1)*25);}
            }
            fop.flush();
            fop.close();
        } catch (Exception e) {
    ...
    // of W capacity
    static long unboundedKnapsack(long W, long n, long[] val, long[] wt) {
        ...
        for (int i = 0; i <= W; i++) {
            for (int j = 0; j < n; j++) {
                if (wt[j] <= i) {
                    dp[i] = Math.max(dp[i], dp[(int) (i - wt[j])] + val[j]);
                }
            }
        }
        return dp[(int) W];
