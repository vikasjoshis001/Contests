...
class Main {
    ...
    /* docstring*/
    static void PolycarpandCoins() throws IOException{
    ...
    /* docstring*/
    static void WonderfulColoring1() throws IOException{
        int T = Integer.parseInt(sc.next());
        while (T-- > 0) {
            // Write Your Code...
            ...
            String rail = sc.next();
            int ans=0,newans=0;
            // char[] rali =  new char[rail.length()] ;
            // rali = rail.toCharArray();
            if(rail.length() == 1){
                System.out.println(0);
                continue;
            }
            for (int i = 0; i < rail.length(); i++) {
                if(mp.containsKey(rail.charAt(i))) {
                    mp.put(rail.charAt(i), mp.get(rail.charAt(i)) + 1);
                }
                else{
                    mp.put(rail.charAt(i), 1);
                }
            }
            for (int c : mp.values()){
                if(c >=2){
