...
class CodeChef {
    ...
    /* docstring*/
    static void EITA() throws IOException{
        int T = Integer.parseInt(sc.next());
        while (T-- > 0) {
            // Write Your Code...
            int arr[] = sc.readArray(4);
            int firstStrategi = arr[1]*7;
            int secondStrategi = (arr[2]*arr[0]) + ( (7-arr[0])*arr[3]);
            System.out.println(Math.max(firstStrategi,secondStrategi));
        }
        fop.flush();
        fop.close();
    }
    ...
    /* docstring*/
    static void RELATIVE() throws IOException{
        int T = Integer.parseInt(sc.next());
        while (T-- > 0) {
            // Write Your Code...
            int arr[] = sc.readArray(2);
            System.out.println( (arr[1]*arr[1]) / (2*arr[0]) );
        }
        fop.flush();
        fop.close();
    }
