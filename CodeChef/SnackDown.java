...
class CodeChef {
    ...
    /* docstring */
    static void QUALPREL() throws IOException {
        ...
        while (T-- > 0) {
            ...
            Arrays.sort(arr);
            int reversed_arr[] = Templates.reverse(arr, arr.length);
            int val = reversed_arr[K - 1];
            for (int i = 0; i < reversed_arr.length; i++) {
                if (reversed_arr[i] >= val) {
                    count++;
                }
            }
    ...
    /* docstring */
    static void TEMPLELA() throws IOException {
        int T = Integer.parseInt(sc.next());
        while (T-- > 0) {
            // Write Your Code...
            int N = sc.nextInt();
    ...
    /* docstring */
    static void KTTABLE() throws IOException {
        int T = Integer.parseInt(sc.next());
        while (T-- > 0) {
            // Write Your Code...
            int N = sc.nextInt();
            int assigned_time[] = sc.readArray(N);
            int requirred_time[] = sc.readArray(N);
            int start = 0;
            int count = 0;
            for (int i = 0; i < N; i++) {
                int total_time = assigned_time[i] - start;
                if (total_time >= requirred_time[i]) {
                    count++;
                }
                start = assigned_time[i];
            }
            System.out.println(count);
        }
        fop.flush();
    ...
    /* docstring */
    static void SNAKPROC() throws IOException {
        int T = Integer.parseInt(sc.next());
        while (T-- > 0) {
            // Write Your Code...
            int N = sc.nextInt();
